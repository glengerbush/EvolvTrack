...
describe('Health Data Storage change events', () => {
  it('emits add then patch then delete for a weight lifecycle', async () => {
    ...
    try {
      ...
      expect(events[1]).toMatchObject({ action: 'patch', id: created.id });
      ...
      expect(events[2]).toMatchObject({ action: 'delete', id: created.id });
    } finally {
      unsubscribe();
    }
  });
  ...
  it('clearAllData emits a single entries reset', async () => {
    const { events, unsubscribe } = captureChanges();
    try {
      await clearAllData();
  ...
  // updateEntry per row in a sequential await loop, which produced one
  // transaction and one store-rebuilding event per row. The bulk variant must
  // collapse both: every patched row persists, and listeners see a single
  // bulkPatch event that names every id.
  it('bulkUpdateEntries persists every patched row and emits exactly one bulkPatch event', async () => {
    const created = await Promise.all([
      addEntry({ date: TODAY, amountMg: 1, medication: '', site: '', symptoms: [] }),
      addEntry({ date: TODAY, amountMg: 2, medication: '', site: '', symptoms: [] }),
      addEntry({ date: TODAY, amountMg: 3, medication: '', site: '', symptoms: [] }),
    ]);
    const ids = created.map((i) => i.id);

    const { events, unsubscribe } = captureChanges();
    try {
      await bulkUpdateEntries(ids, { medication: SEMA });

      expect(events).toHaveLength(1);
      const event = events[0];
      expect(event).toMatchObject({ action: 'bulkPatch' });
      ...
      const persisted = await getAllEntries();
  ...
  it('bulkUpdateInjections with an empty id list emits nothing and writes nothing', async () => {
    ...
    const { events, unsubscribe } = captureChanges();
    try {
      await bulkUpdateEntries([], { medication: 'Tirzepatide (Mounjaro / Zepbound)' });
      expect(events).toHaveLength(0);
      const persisted = await getAllEntries();
